...
export const normalizeBuckets = (buckets: SeqapiV1HistogramBucketDto[], tz: string, width?: number) => {
	return buckets.reduce((acc, cur) => {
		try {
			const date = new Date(Number(cur.key))
			return {
				// 2013-10-04 22:23:00.000
				x: [...acc.x, formatHistogramDate(date, tz)],
				// initial timestamp for timezones
				_x: [...acc._x, date],
